...
public class TcpServer {
	...
	public ArrayList<FtpHandler> users = new ArrayList();
	public static int counter = 0;
	public static String initDir = System.getProperty("user.dir").replace('\\','/')+ "/ftp";
    static public ArrayList<UserInfo> usersInfo=new ArrayList();
    
	public TcpServer() {
		
		usersInfo.add(new UserInfo("lihuan", "lihuan", System.getProperty("user.dir").replace('\\', '/')+"/ftp"));
	...
	class FtpHandler extends Thread {
		...
		String dir = TcpServer.initDir;// 当前目录
		String rootdir = System.getProperty("user.dir").replace('\\', '/')+"/ftp"; // 默认根目录
		...
		{
			for (int i = 0; i < TcpServer.usersInfo.size(); i++) {
				if (((UserInfo) TcpServer.usersInfo.get(i)).user.equals(user)
						&& ((UserInfo) TcpServer.usersInfo.get(i)).password
								.equals(s)) {
					rootdir = ((UserInfo) TcpServer.usersInfo.get(i)).workDir;
					dir = ((UserInfo) TcpServer.usersInfo.get(i)).workDir;
					return true;
				}
			}
			return false;
